def caesar():
    ...
    s="Result: "
    if len(alpha)<1:                 #accounts for if the user enters nothing at all
        s=""
        s+="You did not enter anything"
    for i in range(len(alpha)):
        if  alpha[i].lower() not in bet and alpha[i] != " ":     #accounts for if the user enters a number or special character
            s=""
            s+="Invalid input!!! Enter words consisting of only letters"
            break
        else:
            if alpha[i] == " ":      #accounts for space entries
                s+=" "
